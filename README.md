...
Система объединяет **линейное программирование** (симплекс-метод) для минимизации стоимости кормов при соблюдении зоотехнических норм и **ИИ-агента** на базе локальных языковых моделей (Qwen 3.5) с архитектурой RAG и поддержкой вызова инструментов (Tool Calling). Всё работает **локально на вашем компьютере** — интернет не требуется.
...
2. Скачайте **`Felex_1.0.0_x64_portable.zip`** (6.7 МБ)
3. Распакуйте в любую папку
4. Запустите **`Felex.exe`**

> Установка не требуется. Работает из любой директории.

### Вариант 2: Установщик EXE

1. Скачайте **`Felex_1.0.0_x64-setup.exe`** (744 КБ) из [Релизов](https://github.com/danilkotelnikov/Felex/releases/latest)
2. Запустите — следуйте мастеру установки (2 клика)
3. Запустите **Felex** из меню «Пуск»

### Вариант 3: MSI-установщик

Скачайте **`Felex_1.0.0_x64_ru-RU.msi`** (русский, 1.1 МБ) или **`Felex_1.0.0_x64_en-US.msi`** (английский) из [Релизов](https://github.com/danilkotelnikov/Felex/releases/latest).
...
Решатель: **MinLP** (пересмотренный симплекс-метод, разреженные матрицы). Время решения: **2–26 мс** для рационов с 5–22 кормами.

---

## Бенчмарки производительности

Тестовая конфигурация: AMD Ryzen 7 8845HS, 16 ГБ DDR5, NVIDIA RTX 4060 Laptop (8 ГБ VRAM), NVMe SSD, Windows 11 Pro.

### База данных (HTTP API, SQLite WAL)

| Операция | Мин | Средн | Макс |
|---|---|---|---|
| Полная выборка 300 кормов | 11.7 мс | 12.5 мс | 13.1 мс |
| Поиск корма по ключевому слову | 2.5 мс | 2.7 мс | 23.4 мс* |
| Получение одного корма | 1.7 мс | 2.0 мс | 2.3 мс |
| Расчёт нутриентного профиля | 2.9 мс | 4.4 мс | 7.1 мс |
| Экономический анализ | 3.2 мс | 5.3 мс | 12.1 мс |

\* Первый запрос — холодный кэш

### LP-оптимизатор

| Сценарий | Кормов | Режим | Средн (мс) |
|---|---|---|---|
| Простой молочный рацион | 5 | MinimizeCost | 2.4 |
| Простой молочный рацион | 5 | BalanceNutrients | 9.4 |
| Типичный молочный рацион | 12 | MinimizeCost | 3.4 |
| Типичный молочный рацион | 12 | BalanceNutrients | 7.8 |
| Сложный рацион | 22 | MinimizeCost | 5.3 |
| Сложный рацион | 22 | BalanceNutrients | 11.5 |
| Откорм свиней | 8 | MinimizeCost | 7.2 |
| Бройлер | 10 | MinimizeCost | 3.2 |

### ИИ-агент (Ollama + Qwen 3.5:4b)

| Тест | Время | Качество |
|---|---|---|
| Корма с лизином для свиней | 39.5 с | Корректный список (соя, рыбная мука) |
| Дефицит Ca у коровы | 42.9 с | Верные рекомендации (мел, дикальцийфосфат) |
| Соотношение Ca:P | 30.6 с | Научное объяснение с таблицей |
| Оптимизация бройлера | >120 с | Таймаут (сложный запрос) |
| Сырой картофель свиньям | 25.1 с | Корректное предупреждение о соланине |

### Общие показатели

| Метрика | Значение |
|---|---|
| Холодный старт до готовности API | 3–5 с |
| Размер установщика NSIS | 744 КБ |
| Размер установщика MSI | 1.1 МБ |
| Portable-архив | 6.7 МБ |
| Потребление RAM (без LLM) | 35–45 МБ |
| Потребление RAM (с LLM 4B) | ~4.2 ГБ |
| Бандл JS + CSS | 367 КБ + 5.4 КБ |
...
|---|---|---|---|
| Qwen 3.5 4B | 3.4 ГБ | 25–43 с | 8 ГБ RAM + 8 ГБ VRAM |
| Qwen 3.5 9B | 6.6 ГБ | — | 16 ГБ RAM + 12 ГБ VRAM |
